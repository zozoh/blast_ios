...
- (id)initWithAnnotation:(id<MKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier {
    if(self = [super initWithAnnotation:annotation reuseIdentifier:reuseIdentifier]) {
        ...
        if(self.shouldBeFlat) {
            ...
            self.bounds = CGRectMake(0, 0, 13, 13);
...
- (CALayer *)colorHaloLayer {
    if(!_colorHaloLayer) {
        ...
        float size = ((SVAnnotation*)self.annotation).size;

        _colorHaloLayer.bounds = CGRectMake(0, 0, size, size);
        ...
        _colorHaloLayer.cornerRadius = size / 2;
